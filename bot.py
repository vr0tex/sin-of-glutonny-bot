...
class ApplyView(discord.ui.View):
    ...
    async def get_mod_app_channel(self, interaction: discord.Interaction):
        candidates = []

        if interaction.guild:
            candidates.append(interaction.guild.get_channel(MOD_APP_CHANNEL_ID))

        candidates.append(bot.get_channel(MOD_APP_CHANNEL_ID))

        for guild in bot.guilds:
            channel = guild.get_channel(MOD_APP_CHANNEL_ID)
            if channel:
